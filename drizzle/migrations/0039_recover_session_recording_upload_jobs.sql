...
-- Recovery for rows that failed before ffmpeg was available and/or before the
-- recording-parts table existed. This keeps the parent job unique by
-- session_id + zoom_meeting_id, refreshes its aggregate manifest from parts,
-- and makes retryable failed jobs visible to the worker again.
...
WHERE
    ...
    AND zoom_recording_manifest IS NOT NULL
    AND jsonb_array_length(zoom_recording_manifest) > 0;
