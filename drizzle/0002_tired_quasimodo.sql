...
SELECT id, user_id, zuvy_test_cases_submission, created_at
FROM zuvy_practice_code
WHERE user_id = 63619
...
FROM zuvy_outsourse_coding_questions
WHERE id IN (2859, 2860, 2861);
...
SELECT *
FROM zuvy_outsourse_coding_questions
WHERE assessment_outsourse_id IN (
  996,
  2364,
  1866,
  2446,
...
SELECT
    ...
    created_at
FROM zuvy_practice_code
WHERE question_id IN (284, 617, 484)
ORDER BY id DESC;
...
SELECT
  pc.id AS "practiceCodeId",
  pc."userId",
  pc."codingOutsourseId",
  pc.status,
  cq.id AS "codingQuestionId",
  cq."chapterId",
  mc.id AS "chapterId",
  mc."moduleId",
  mc."topicId"
FROM zuvy_practice_code pc
LEFT JOIN zuvy_outsourse_coding_questions cq
  ON pc."codingOutsourseId" = cq.id
LEFT JOIN zuvy_module_chapter mc
  ON cq."chapterId" = mc.id
WHERE pc.id = 2147;