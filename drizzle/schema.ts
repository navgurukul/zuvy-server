import { pgTable, pgSchema,jsonb, foreignKey, unique, pgEnum, serial, varchar, integer, timestamp, boolean, date, index, bigserial, bigint, char, text, doublePrecision, uniqueIndex, json, customType } from "drizzle-orm/pg-core"
...
export const exercisesReviewType = pgEnum("main.exercises_review_type", ['automatic', 'facilitator', 'peer', 'manual'])
export const exercisesSubmissionType = pgEnum("main.exercises_submission_type", ['url', 'attachments', 'text_large', 'text', 'number'])
export const submissionsState = pgEnum("main.submissions_state", ['rejected', 'pending', 'completed'])
export const userRolesCenter = pgEnum("main.user_roles_center", ['all', 'banagalore', 'dharamshala'])
export const userRolesRoles = pgEnum("main.user_roles_roles", ['facilitator', 'student', 'alumni', 'admin','instractor'])
export const usersCenter = pgEnum("main.users_center", ['bangalore', 'dharamshala'])
...
export const exercises = main.table("exercises", {
	...
	sequenceNum: doublePrecision("sequence_num"),
	// TODO: failed to parse database type 'main.exercises_review_type'
	reviewType: exercisesReviewType("main.exercises_review_type"),
	content: text("content"),
	// TODO: failed to parse database type 'main.exercises_submission_type'
	submissionType: exercisesSubmissionType("main.exercises_submission_type"),
...
export const hackathonCourses = main.table("hackathon_courses", {
	id: serial("id").primaryKey().notNull(),
	email: varchar("email", { length: 255 }).notNull(),
	courseId: integer("course_id").notNull(),
	exerciseId: integer("exercise_id"),
...
export const hackathonLogin = main.table("hackathon_login", {
	id: serial("id").primaryKey().notNull(),
	name: varchar("name", { length: 255 }).notNull(),
	email: varchar("email", { length: 255 }).notNull(),
	password: varchar("password", { length: 255 }).notNull(),
...
export const hackathonQuestions = main.table("hackathon_questions", {
	id: serial("id").primaryKey().notNull(),
	email: varchar("email", { length: 255 }).notNull(),
	questions: varchar("questions", { length: 500 }).notNull(),
});
...
export const userRoles = main.table("user_roles", {
	...
	// TODO: failed to parse database type 'main.user_roles_roles'
	roles: userRolesRoles("main.user_roles_roles"),
	// TODO: failed to parse database type 'main.user_roles_center'
	center: userRolesCenter("main.user_roles_center"),
},
(table) => {
	return {
...
export const users = main.table("users", {
	...
	// TODO: failed to parse database type 'main.users_center'
	center: usersCenter("main.users_center"),
...
export const englishAiContent = main.table("english_ai_content", {
	id: serial("id").primaryKey().notNull(),
...
export const englishAiHistory = main.table("english_ai_history", {
	id: serial("id").primaryKey().notNull(),
	userId: integer("user_id").notNull().references(() => users.id),
	englishAiId: integer("english_ai_id").notNull().references(() => englishAiContent.id),
	createdAt: timestamp("createdAt", { withTimezone: true, mode: 'string' }).notNull(),
});

export const hackathonAnswers = main.table("hackathon_answers", {
	id: serial("id").primaryKey().notNull(),
	questionId: integer("question_id").notNull().references(() => hackathonQuestions.id),
	answers: varchar("answers", { length: 500 }).notNull(),
	email: varchar("email", { length: 255 }),
});
