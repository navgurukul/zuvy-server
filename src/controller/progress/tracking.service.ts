...
import { eq, sql, inArray, and, SQL, asc, desc, count } from 'drizzle-orm';
...
export class TrackingService {
    ...
    try {
      ...
      if (chapterExistsInModuleChapter.length != 0) {
        if (chapterExistsInChapterTracking.length == 0) {
          ...
          const recentBootcampForUser = await db
            .select()
          ...
          const projectModuleIds =
            projectModules.length > 0
              ...
              : [];
          const completedProjectForAUser =
            projectModuleIds.length > 0
              ? await db
                  .select()
                  .from(zuvyModuleTracking)
                  .where(
                    sql`${inArray(zuvyModuleTracking.moduleId, projectModuleIds)} and ${zuvyModuleTracking.userId} = ${userId}`,
                  )
              : [];
          ...
          if (userBootcampTracking.length > 0) {
            const updatedBootcampProgress = await db
              ...
              .set({
                progress: Math.ceil(
                  ((allChapterTracking.length +
                    completedProjectForAUser.length) /
                    (allChapters.length + projectModules.length)) *
                    100,
                ),
                updatedAt: sql`NOW()`,
              })
  ...
  async getAllModuleByBootcampIdForStudent(bootcampId: number, userId: number) {
    try {
      ...
      const modulesData = await db.query.zuvyCourseModules.findMany({
      ...
      const chapterTopicCounts = await db
        .select({
        ...
        .groupBy(zuvyModuleChapter.moduleId, zuvyModuleChapter.topicId);

      const completedChapterCounts = await db
        .select({
          moduleId: zuvyModuleChapter.moduleId,
          completed: count(),
        })
      ...
      const firstChapterRows = await db
        ...
        .from(zuvyModuleChapter)
        .where(inArray(zuvyModuleChapter.moduleId, moduleIds))
        .orderBy(asc(zuvyModuleChapter.moduleId), asc(zuvyModuleChapter.order));

      const chapterCountsByModule = new Map<
        number,
        { total: number; byTopic: Record<number, number> }
      >();
      for (const row of chapterTopicCounts) {
        const moduleId = Number((row as any).moduleId);
        const topicId = Number((row as any).topicId);
        const countValue = Number((row as any).count ?? 0);

        const previous = chapterCountsByModule.get(moduleId) ?? {
          total: 0,
          byTopic: {},
        };
        previous.total += countValue;
        previous.byTopic[topicId] = countValue;
        chapterCountsByModule.set(moduleId, previous);
      }

      const completedMap = new Map<number, number>();
      for (const row of completedChapterCounts) {
        completedMap.set(
          Number((row as any).moduleId),
          Number((row as any).completed ?? 0),
        );
      }

      const firstChapterByModule = new Map<number, number>();
      for (const row of firstChapterRows) {
        const moduleId = Number((row as any).moduleId);
        if (!firstChapterByModule.has(moduleId)) {
          firstChapterByModule.set(moduleId, Number((row as any).chapterId));
        }
      }

      const progressUpdates: Array<{ id: number; progress: number }> = [];
      const updatedProgressMap = new Map<number, number>();
      ...
      for (const module of modulesData) {
        ...
        const currentTracking = module.moduleTracking[0];
        const chapterCounts = chapterCountsByModule.get(module.id);
        const totalChapters = chapterCounts?.total ?? 0;

        if (!currentTracking || totalChapters === 0) {
          continue;
        }

        const completedChapters = completedMap.get(module.id) ?? 0;
        const calculatedProgress = Math.ceil(
          (completedChapters / totalChapters) * 100,
        );

        if (currentTracking.progress !== calculatedProgress) {
          progressUpdates.push({
            id: currentTracking.id,
            progress: calculatedProgress,
          });
          updatedProgressMap.set(module.id, calculatedProgress);
        }
      }

      if (progressUpdates.length > 0) {
        await Promise.all(
          progressUpdates.map(({ id, progress }) =>
            db
              .update(zuvyModuleTracking)
              .set({ progress })
              .where(eq(zuvyModuleTracking.id, id)),
          ),
        );
      }
      ...
      let modules = modulesData.map((module) => {
        const chapterCounts = chapterCountsByModule.get(module.id);
        const byTopic = chapterCounts?.byTopic ?? {};
        const currentTracking = module.moduleTracking[0];
        ...
        return {
          ...
          progress:
            updatedProgressMap.get(module.id) ?? currentTracking?.progress ?? 0,
          ChapterId: firstChapterByModule.get(module.id) ?? null,
          quizCount: byTopic[4] ?? 0,
          assignmentCount: byTopic[5] ?? 0,
          codingProblemsCount: byTopic[3] ?? 0,
          articlesCount: byTopic[2] ?? 0,
          formCount: byTopic[7] ?? 0,
        };
      });
    ...
    try {
      ...
      if (allChapters != 0 || projectModules.length != 0) {
        initialProgress = Math.ceil(
          ((allChapterTracking.length + completedProjectForAUser.length) /
                  ...
                  with: {
                    ...
                    quizTrackingData: {
                      ...
                      where: (quizTracking, { sql }) =>
                        sql`${quizTracking.userId} = ${userId} and ${quizTracking.chapterId} = ${chapterId} and ${quizTracking.moduleId} = ${moduleId}`,
                    },
                  },
                });
          ...
          if (chapter[0].quizQuestions !== null) {
            ...
            if (isAttempted) {
              ...
              const allVariants =
                await db.query.zuvyModuleQuizVariants.findMany({
                  where: (moduleQuiz, { sql }) =>
                    sql`${inArray(moduleQuiz.quizId, quizQuestionIds)}`,
  ...
  async submitProjectForAUser(
    ...
    try {
      ...
      if (projectTrackingForUser.length > 0) {
        ...
        const projectTracked = await db
          .insert(zuvyProjectTracking)
          .values(updatedBody)
          .returning();
        const moduleTracked = await db
          .insert(zuvyModuleTracking)
