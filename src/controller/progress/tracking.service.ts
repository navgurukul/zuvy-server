...
import { SubmitBodyDto } from './dto/assignment.dto';
...
export class TrackingService {
  ...
  async recomputeBatchAttendancePercentages(batchId: number) {
    try {
      ...
      await db
        .update(zuvyBatchEnrollments)
        .set({ attendance: finalSql })
        .where(sql`${inArray(zuvyBatchEnrollments.id, ids)}`);

      return { success: true, updated: ids.length };
    } catch (err: any) {
      if (this.logger)
        this.logger.error(
          `recomputeBatchAttendancePercentages failed: ${err?.message ?? err}`,
        );
    ...
    try {
      ...
      if (chapterExistsInModuleChapter.length != 0) {
        if (chapterExistsInChapterTracking.length == 0) {
          ...
          const [
            moduleTracking,
            totalModuleChapterCount,
            completedModuleChapterCount,
          ] = await Promise.all([
            db
              .select({ id: zuvyModuleTracking.id })
              .from(zuvyModuleTracking)
              .where(
                and(
                  eq(zuvyModuleTracking.userId, userId),
                  eq(zuvyModuleTracking.bootcampId, bootcampId),
          ...
          let totalChapter = totalModuleChapterCount[0].count;
          let completedChapter = completedModuleChapterCount[0].count;
          const moduleProgress = Math.ceil(
            (completedChapter / totalChapter) * 100,
          );

          const moduleTrackingUpsert =
            moduleTracking.length == 0
              ? db
          ...
          // The moduleTracking upsert, the recent-bootcamp lookup and the
          // totalModules lookup don't read each other's results - run them
          // together.
          const [returnedTrackingData, recentBootcampForUser, totalModules] =
            await Promise.all([
              moduleTrackingUpsert,
              db
                .select({ id: zuvyRecentBootcamp.id })
                .from(zuvyRecentBootcamp)
                .where(eq(zuvyRecentBootcamp.userId, BigInt(userId))),
              db
                .select({
                  id: zuvyCourseModules.id,
                  typeId: zuvyCourseModules.typeId,
                })
                .from(zuvyCourseModules)
                .where(eq(zuvyCourseModules.bootcampId, bootcampId)),
            ]);

          const projectModules = totalModules.filter(
            (module) => module.typeId === 2,
          );
          const projectModuleIds =
            projectModules.length > 0
              ? projectModules.map((module) => module.id)
              : [];
          const moduleIds = totalModules.map((module) => module.id);

          const recentBootcampUpsert =
            recentBootcampForUser.length == 0
              ? db.insert(zuvyRecentBootcamp).values({
                  userId: BigInt(userId),
                  moduleId,
                  progress: moduleProgress,
                  bootcampId,
          ...
          const [
            ...
            projectModuleIds.length > 0
              ? db
                  ...
                  .where(
                    and(
                      inArray(zuvyModuleTracking.moduleId, projectModuleIds),
                      eq(zuvyModuleTracking.userId, userId),
                    ),
                  )
          ...
          const bootcampProgress = Math.ceil(
            ((allChapterTrackingCount + completedProjectForAUserCount) /
              (allChaptersCount + projectModules.length)) *
              100,
          );
    ...
    try {
      ...
      if (moduleDetails.length > 0) {
        ...
        if (assessmentChapterIds.length > 0) {
          ...
          // perf: parallelized - each assessment's state update is independent,
          // so run them together instead of awaiting one at a time in a loop.
          await Promise.all(
            assessments.map((assessment) =>
              this.contentService.updateAssessmentState(assessment),
            ),
          );
    ...
    try {
      ...
      if (SubmitBody.submitAssignment != undefined) {
        ...
        const questions = await db
          ...
          .where(inArray(zuvyModuleQuizVariants.id, mcqIdArray));

        // Prepare the data for insertion
        const insertData = questions.map((question, index) => {
          const chosenOption = chosenOptions[index];
          const status =
            chosenOption === question.correctOption ? 'pass' : 'fail';
        ...
        await db.insert(zuvyQuizTracking).values(insertData);
  ...
  async getAllModuleByBootcampIdForStudent(bootcampId: number, userId: number) {
    try {
      ...
      // The only thing chapterTrackingDetails was used for was counting, per
      // module, how many of its chapters the user has completed - so fetch
      // that as a single grouped count instead of every chapter's tracking
      // rows nested under every chapter.
      const moduleIds = data.map((module) => module.id);
      const completedChapterCounts =
        moduleIds.length > 0
          ? await db
              .select({
              ...
              .innerJoin(
                zuvyModuleChapter,
                eq(zuvyModuleChapter.id, zuvyChapterTracking.chapterId),
              )
              .where(
                and(
                  eq(zuvyChapterTracking.userId, BigInt(userId)),
              ...
              .groupBy(zuvyModuleChapter.moduleId)
          : [];
      const completedChaptersByModule = new Map(
        completedChapterCounts.map((r) => [r.moduleId, r.count]),
      );
      ...
      if (result.length > 0) {
        ...
        for (const input of result) {
          sqlChunks.push(
            sql`when ${zuvyModuleTracking.id} = ${input.id} then ${sql.raw(`CAST(${input.progress} AS INTEGER)`)}`,
          );
          ids.push(input.id);
        }
        ...
        if (!isCourseLocked) {
          ...
          if (lastStartedOrCompletedIndex === -1) {
            ...
            const isLastCompleted =
              modules[lastStartedOrCompletedIndex].progress === 100;
            ...
            modules = modules.map((module, index) => {
              ...
              if (
                isLastCompleted &&
                index === lastStartedOrCompletedIndex + 1
              ) {
  ...
  async getBootcampTrackingForAUser(bootcampId: number, userId: number) {
    try {
      // These two lookups don't depend on each other.
      const [bootcampInfo, moduleDetails] = await Promise.all([
      ...
      // Doesn't depend on any of the progress computation below - kick it
      // off now and only await it once it's actually needed for the response.
      const batchDetailsPromise = db.query.zuvyBatchEnrollments.findFirst({
        where: (batchEnroll, { and, eq }) =>
          and(
            eq(batchEnroll.userId, BigInt(userId)),
            eq(batchEnroll.bootcampId, bootcampId),
          ),
        with: {
          batchInfo: {
            columns: {
      ...
      const projectModules = moduleDetails.filter(
        (module) => module.typeId === 2,
      );
      const projectModuleIds =
        projectModules.length > 0
      ...
      if (allChapters != 0 || projectModules.length != 0) {
        initialProgress = Math.ceil(
          ((allChapterTrackingCount + completedProjectForAUserCount) /
            (allChapters + projectModules.length)) *
            100,
        );
      }
      // Single upsert against the (userId, bootcampId) unique constraint
      ...
      const pendingModules = await db
        ...
        .where(
          and(
            eq(zuvyModuleTracking.bootcampId, bootcampId),
            eq(zuvyModuleTracking.userId, userId),
            lt(zuvyModuleTracking.progress, 100),
          ),
        );
      const moduleIds = pendingModules.map((obj) => obj.moduleId);
      if (pendingModules.length > 0) {
        const data = await db.query.zuvyModuleChapter.findMany({
          where: (moduleChapter, { inArray, eq, and }) =>
            and(
              inArray(moduleChapter.moduleId, moduleIds),
  ...
  async getAllUpcomingSubmission(
    userId: number,
    bootcampId: number,
  ): Promise<any> {
    try {
          ...
          bootcamp: {
            ...
            with: {
              bootcampModules: {
                ...
                with: {
                  moduleTracking: {
                    where: (moduleTracked, { and, eq, lt }) =>
                      and(
                        eq(moduleTracked.userId, userId),
                        lt(moduleTracked.progress, 100),
                      ),
                    columns: {
                      id: true,
                      progress: true,
      ...
      if (data.length > 0) {
        ...
        if (chapterIds.length > 0) {
          const completedChapterIds = await db
            .select({ id: zuvyChapterTracking.chapterId })
            .from(zuvyChapterTracking)
            .where(
              and(
                inArray(zuvyChapterTracking.chapterId, chapterIds),
                eq(zuvyChapterTracking.userId, BigInt(userId)),
              ),
            );

          const completedChapterIdsSet = new Set(
            completedChapterIds.map((chapter) => chapter.id),
          );
        ...
        return [
          { message: 'No content found', statusCode: STATUS_CODES.NO_CONTENT },
          null,
        ];
      }
      ...
      return [
        null,
        {
          message: 'Upcoming submission fetched successfully',
          statusCode: STATUS_CODES.OK,
          data: { upcomingAssignments, lateAssignments },
        },
      ];
    } catch (error) {
      return [
    ...
    try {
      ...
      if (session) {
        if (
          ...
          session.isZoomMeet &&
          !session.s3link.includes('www.youtube.com')
        ) {
          const updatedRecordingLink =
            await this.zoomService.getMeetingRecordingLink(
              session.zoomMeetingId,
            );
          if (updatedRecordingLink.success) {
            session.s3link =
              updatedRecordingLink.data.playUrl !== null
                ? updatedRecordingLink.data.playUrl
                : session.s3link;
          }
        }
      ...
      if (
        ...
        trackingData['sessions'][0].status === 'completed'
      ) {
        if (trackingData['sessions'][0].attendance === 'present') {
          await this.updateChapterStatus(
            trackingData['sessions'][0].bootcampId,
            userId,
            trackingData['moduleId'],
            chapterId,
          );
          trackingData['status'] = 'Completed';
          const newLiveChapterStatusId = await db
            .select({ id: zuvyChapterTracking.id })
            .from(zuvyChapterTracking)
            .where(
              and(
  ...
  async getAllQuizAndAssignmentWithStatus(
    ...
    try {
      // These four lookups don't depend on each other - run them together.
      const [
        chapterDetails,
        AssignmentTracking,
        QuizTracking,
        codingQuestionTracking,
      ] = await Promise.all([
        db
          .select()
          .from(zuvyModuleChapter)
        ...
        db
          ...
          .where(
            and(
              ...
              eq(zuvyAssignmentSubmission.chapterId, chapterId),
              eq(zuvyAssignmentSubmission.moduleId, moduleId),
            ),
          ),
        db
          .select()
                ...
                await db.query.zuvyModuleQuizVariants.findMany({
                  ...
                  with: {
                    ...
                    quizTrackingData: {
                      ...
                      where: (quizTracking, { and, eq }) =>
                        and(
                          eq(quizTracking.userId, userId),
                          eq(quizTracking.chapterId, chapterId),
                          eq(quizTracking.moduleId, moduleId),
                        ),
                    },
                  },
                });
          ...
          if (chapterDetails[0].codingQuestions !== null) {
            if (codingQuestionTracking.length == 0) {
              ...
              codingProblemDetails['status'] =
                codingQuestionTracking.length != 0 ? 'Completed' : 'Pending';
      ...
      if (chapter.length > 0) {
        if (chapter[0].topicId == 4) {
          if (chapter[0].quizQuestions !== null) {
            ...
            // Check if chapter was tracked - independent of the quiz-attempt
            // lookup below, so run them together.
            const [chapterTracking, quizTrack] = await Promise.all([
              db
                .select()
                .from(zuvyChapterTracking)
                .where(
              ...
              db
                ...
                .where(
                  and(
                    eq(zuvyQuizTracking.userId, userId),
                    eq(zuvyQuizTracking.chapterId, chapterId),
                    isNotNull(zuvyQuizTracking.chosenOption),
                  ),
                ),
            ]);
            ...
            if (isAttempted) {
              ...
              const allVariants =
                await db.query.zuvyModuleQuizVariants.findMany({
                  where: (moduleQuiz, { inArray }) =>
                    inArray(moduleQuiz.quizId, quizQuestionIds),
              ...
              const enrichedVariants = allVariants.map((variant) => ({
                ...variant,
                quizTrackingData:
                  variant['quizTrackingData'].length > 0
                    ? variant['quizTrackingData']
                    : [],
              }));
            ...
            // Fetch all variants for every quiz question in a single query
            // (instead of one query per quizId) and group them by quizId.
            const allVariantsForQuestions =
              await db.query.zuvyModuleQuizVariants.findMany({
                where: (moduleQuiz) =>
                  inArray(moduleQuiz.quizId, quizQuestionIds),
              });
            const variantsByQuizId = new Map<
              number,
              typeof allVariantsForQuestions
            >();
            for (const variant of allVariantsForQuestions) {
              const list = variantsByQuizId.get(variant.quizId) ?? [];
              list.push(variant);
              variantsByQuizId.set(variant.quizId, list);
            }

            const randomVariants = quizQuestionIds.map((quizId) => {
              const allVariants = variantsByQuizId.get(quizId) ?? [];
          ...
          const status =
            assignmentTracking.length > 0 ? 'Completed' : 'Pending';

          return [
            null,
            {
              message: 'Assignment chapter fetched succesfully',
              statusCode: STATUS_CODES.OK,
              data: { chapterDetails: chapter[0], assignmentTracking, status },
            },
          ];
          ...
          const codingProblem = await db
            .select()
            .from(zuvyCodingQuestions)
            .where(eq(zuvyCodingQuestions.id, chapter[0].codingQuestions));
          if (chapter[0].codingQuestions != null && codingProblem.length > 0) {
            return [
              null,
              {
                message: 'Coding chapter fetched succesfully',
                statusCode: STATUS_CODES.OK,
                data: { chapterDetails: chapter[0], codingProblem, status },
              },
            ];
          } else {
            return [
              null,
              {
                message: 'There is no coding question in this chapter',
                statusCode: STATUS_CODES.OK,
                data: null,
              },
            ];
          ...
          return [
            null,
            {
              message: 'It is not a quiz or assignment chapter',
              statusCode: STATUS_CODES.OK,
              data: null,
            },
          ];
    ...
    try {
      const projectTrackingForUser = await db
        ...
        .where(
          and(
            ...
            eq(zuvyProjectTracking.moduleId, moduleId),
      ...
      if (projectTrackingForUser.length > 0) {
        let updateProject = await db
          .update(zuvyProjectTracking)
          .set(updatedBody)
          .where(eq(zuvyProjectTracking.id, projectTrackingForUser[0].id))
          .returning();
        if (updateProject.length > 0) {
          return {
            ...
            code: 200,
            message:
              'Your project has been submitted and course progress has been updated successfully',
            updateProject,
          };
        } else {
          return {
            ...
            code: 400,
            message:
              'There is some error while submitting your project.Try again',
          };
        }
      } else {
        const moduleTrackingBody = {
          userId,
          moduleId,
          bootcampId,
          progress: 100,
        ...
        const [recentBootcampForUser, projectTracked, moduleTracked] =
          await Promise.all([
            db
              ...
              .from(zuvyRecentBootcamp)
              .where(eq(zuvyRecentBootcamp.userId, BigInt(userId))),
            db.insert(zuvyProjectTracking).values(updatedBody).returning(),
            db
              .insert(zuvyModuleTracking)
              .values(moduleTrackingBody)
              .returning(),
          ]);
  ...
  async getProjectDetailsWithStatus(
    ...
    try {
      const data = await db.query.zuvyCourseModules.findFirst({
        where: (courseModules, { eq }) => eq(courseModules.id, moduleId),
        with: {
          projectData: {
            where: (projectDetails, { eq }) => eq(projectDetails.id, projectId),
            with: {
              projectTrackingData: {
                columns: {
                  projectLink: true,
                ...
                where: (projectTrack, { eq }) =>
                  eq(projectTrack.userId, userId),
              },
            },
          },
          moduleTracking: {
  ...
  async allBootcampProgressForStudents(userId: number) {
    try {
      const data = await db.query.zuvyBatchEnrollments.findMany({
        ...
        where: (batchEnroll, { eq }) => eq(batchEnroll.userId, BigInt(userId)),
        with: {
          bootcamp: {
            columns: {
              id: true,
  ...
  async getLatestUpdatedCourseForStudents(
    userId: number,
    bootcampId: number,
  ): Promise<any> {
    try {
      // These three don't depend on each other - run them together.
      const [bootcamp, bootcampTypes, modules] = await Promise.all([
        db.select().from(zuvyBootcamps).where(eq(zuvyBootcamps.id, bootcampId)),
        db
          .select({
      ...
      const leaderboardEnabled = bootcampTypes.some(
        (bootcampType) => bootcampType.leaderboardEnabled === true,
      );
      ...
      const [chapterRecsAll, projectRecsAll, moduleRecs] = await Promise.all([
        ...
        db
          ...
          .where(
            and(
              eq(zuvyProjectTracking.userId, userId),
              inArray(zuvyProjectTracking.moduleId, moduleIds),
            ),
          ),
        db
          .select()
          ...
          .where(
            and(
              eq(zuvyModuleTracking.userId, userId),
              inArray(zuvyModuleTracking.moduleId, moduleIds),
            ),
          ),
      ]);

      const trackedChapterIds = new Set(
        chapterRecsAll.map((r) => Number(r.chapterId)),
      );
      const trackedProjMods = new Set(
        projectRecsAll.map((r) => Number(r.moduleId)),
      );
      const moduleProgMap = new Map(
        moduleRecs.map((r) => [Number(r.moduleId), Number(r.progress)]),
      );

      // 4. Helper function to calculate actual module progress based on current chapters vs tracking
      const getActualModuleProgress = async (
        module: (typeof modules)[0],
      ): Promise<{ progress: number; hasAvailableChapters: boolean }> => {
        if (module.typeId === 2) {
          // Project module
          return {
          ...
          const availableChapters = currentChapters.filter(
            (chapter) =>
              !(
                chapter.topicId === 6 &&
                (assessmentStatus.get(chapter.id) ?? 0) !== 1 &&
                (assessmentStatus.get(chapter.id) ?? 0) !== 2
              ),
          );
          ...
          const actualCompletionRate =
            completedChapters.length / availableChapters.length;
      ...
      // Cache each module's computed progress so the later "double-check all
      // complete" fallback loop (step 8) can reuse it instead of re-fetching
      // chapters/assessments from the DB a second time for the same module.
      const moduleProgressCache = new Map<
        number,
        { progress: number; hasAvailableChapters: boolean }
      >();

      for (const module of modules) {
        const moduleProgressResult = await getActualModuleProgress(module);
        moduleProgressCache.set(module.id, moduleProgressResult);
        const { progress, hasAvailableChapters } = moduleProgressResult;
      ...
      if (!currentModule) {
        ...
        for (const module of modules) {
          // Reuse the progress already computed above instead of
          // re-fetching this module's chapters/assessments a second time.
          const { progress, hasAvailableChapters } =
            moduleProgressCache.get(module.id) ??
            (await getActualModuleProgress(module));
      ...
      if (!chapters.length) {
        ...
        console.log(
          `Module ${currentModule.id} is empty, searching for next available module...`,
        );
      ...
      if (!nextChapter) {
        ...
        for (const module of untouchedModules) {
          ...
          if (moduleChapters.length > 0) {
            ...
            const moduleAssessmentRecs =
              moduleChapterIds.length > 0
                ? await db
                    .select()
                    .from(zuvyOutsourseAssessments)
                    .where(
            ...
            for (const chapter of moduleChapters) {
              const isAvailable =
                ...
                !(
                  chapter.topicId === 6 &&
                  (moduleAssessmentStatus.get(chapter.id) ?? 0) !== 1 &&
                  (moduleAssessmentStatus.get(chapter.id) ?? 0) !== 2
                );
  ...
  async formatedChapterDetails(chapterDetails: any) {
    try {
      ...
      chapterDetails.OpenEndedQuestions =
        chapterDetails?.OpenEndedQuestions.map((OpenEndedQuestions) => {
    ...
    try {
      ...
      const data: any = await db.query.zuvyAssessmentSubmission.findFirst({
        ...
        with: {
          ...
          PracticeCode: {
            where: (zuvyPracticeCode, { eq, and, or, ne }) =>
              and(
        ...
        const chapter = await db.query.zuvyModuleChapter.findFirst({
          where: (zuvyModuleChapter, { eq }) =>
            eq(
              ...
              data.submitedOutsourseAssessment.chapterId,
            ),
          columns: {
            id: true,
            title: true,
      ...
      const filteredData = data.PracticeCode.reduce((acc, curr) => {
        const existing = acc.find(
          (item) => item.questionId === curr.questionId,
        );
        ...
        if (!existing) {
          ...
          curr.createdAt > existing.createdAt
        ) {
          acc = acc.filter((item) => item.questionId !== curr.questionId);
          acc.push(curr);
        }
  ...
  async updateFormStatus(
    ...
    try {
      ...
      if (submitFormBody.submitForm !== undefined) {
        ...
        const questionIdArray = submitFormBody.submitForm.map(
          (obj) => obj.questionId,
        );
        const formQuestions = await db
          .select()
          .from(zuvyModuleForm)
          .where(inArray(zuvyModuleForm.id, questionIdArray))
          .orderBy(zuvyModuleForm.id);

        let updatedFormBody = [];
        for (let i = 0; i < formQuestions.length; i++) {
          const chosenOptions = Array.isArray(
            submitFormBody.submitForm[i].chosenOptions,
          )
            ? submitFormBody.submitForm[i].chosenOptions
            : [];
          ...
          let status = 'pending';
          if (chosenOptions !== null || answer !== null) status = 'completed';
