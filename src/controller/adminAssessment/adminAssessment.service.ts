    ...
    try {
      ...
      const combinedData = enrolledStudents.map((student:any) => {
        ...
        return {
          ...
          copyPaste: submission?.copyPaste || 0,
          active: submission?.active || false,
          reattemptRequested: submission?.reattemptRequested || false,
          reattemptApproved: submission?.reattemptApproved || false,
          reattemptCount: (reattemptCountMap.get(userId) || 1) - 1,
          tabChange: submission?.tabChange || 0,
        };
      });
