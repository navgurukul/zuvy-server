...
import { zuvyBatchEnrollments, zuvyChapterTracking, zuvyAssessmentReattempt, zuvyModuleChapter, zuvyOutsourseAssessments, zuvyAssessmentSubmission, users, zuvyModuleQuizVariants, zuvyModuleQuiz, zuvyOpenEndedQuestions, zuvyCodingQuestions, zuvyModuleAssessment, zuvyOutsourseQuizzes, zuvyOutsourseOpenEndedQuestions, zuvyOutsourseCodingQuestions, zuvyTags } from '../../../drizzle/schema';
    ...
    try {
      ...
      const assessmentWithSubmissions = await db
        .select({
          ...
          openEndedQuestion: zuvyOutsourseOpenEndedQuestions.openEndedQuestionId,
        ...
        .leftJoin(
          ...
          eq(zuvyOutsourseCodingQuestions.assessmentOutsourseId, zuvyOutsourseAssessments.id)
        )
        .leftJoin(
          zuvyOutsourseOpenEndedQuestions,
          eq(zuvyOutsourseOpenEndedQuestions.assessmentOutsourseId, zuvyOutsourseAssessments.id)
        )
        .leftJoin(
          zuvyOutsourseQuizzes,
          eq(zuvyOutsourseQuizzes.assessmentOutsourseId, zuvyOutsourseAssessments.id)
        )
        .where(
          assessmentId
            ? eq(zuvyOutsourseAssessments.id, assessmentId)
      ...
      const assessmentName = assessmentDetails[0]?.title || 'Untitled Assessment';
      ...
      let submissions = uniqueSubmissions.filter((s) => (userId ? s.userId === userId : true));
      ...
      if (percentages) {
        const percentageArray = Array.isArray(percentages) ? percentages : [percentages];
      ...
      const percentageStats = sortedBrackets.map((bracket, index) => {
        ...
        uniqueUsers.forEach(userId => {
          ...
          const latestSubmission = userSubmissions.reduce((latest, current) => 
            new Date(current.submitedAt) > new Date(latest.submitedAt) ? current : latest
          );
      ...
      const quizIds = [...new Set(assessmentWithSubmissions
      ...
      const mcqDetails = quizIds.length
        ? await db.select({
            ...
            .leftJoin(zuvyModuleQuiz, eq(zuvyModuleQuiz.id, zuvyModuleQuizVariants.quizId))
      ...
      const tags = tagIds.length
        ? await db.select({ id: zuvyTags.id, name: zuvyTags.tagName }).from(zuvyTags).where(inArray(zuvyTags.id, tagIds))
        : [];

      const mapTagName = (tagId?: number) => tags.find((t) => t.id === tagId)?.name || null;
      ...
      return {
        ...
        assessmentQuestions,
        ...(userId
          ? { userPercentage: submissions[0].percentage, userMarks: submissions[0].marks }
          : {}),
      };
    } catch (error) {
      console.error('Error fetching assessment stats:', error);
      throw new BadRequestException(error.message || 'Failed to fetch assessment stats');
    }
  }
