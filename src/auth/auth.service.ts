...
export class AuthService {
  ...
  constructor(
    ...
    this.logger.log(`Initializing Google Auth Client with ID: ${clientId}`);
  ...
  async login(loginDto: LoginDto) {
    try {
      this.logger.log('Attempting to verify Google ID token');
      this.logger.log(`Using client ID: ${process.env.GOOGLE_CLIENT_ID}`);
      ...
      this.logger.log('Successfully verified Google ID token');
  ...
  async validateToken(token: string) {
    try {
      ...
      if (blacklistedToken) {
        this.logger.log('Token is blacklisted');
