...
export class ScheduleService {
  ...
  async handleDynamicScheduling() {
    ...
    try {
      ...
      this.currentInterval = this.getIntervalBasedOnSessionCount(
        sessions.length,
      );
      this.logger.log(
        `Setting interval to ${this.currentInterval / 60000} minutes`,
      );
  ...
  private async processSessions(sessions: any[]) {
    this.logger.log(
      `Processing ${sessions.length} sessions with delayed execution`,
    );
    let index = 0;
    ...
    try {
      ...
      if (session.meetingId && session.status === 'completed') {
        ...
        if (existing.length === 0) {
          const [errAtten, dataAttendance] = await this.getAttendanceByBatchId(
            session.batchId,
            session.creator,
          );
          if (errAtten) {
  ...
  private async updateSessionLink(calendar: any, session: any) {
    ...
    if (videoAttachment) {
      const updateData: any = { s3link: videoAttachment.fileUrl };
      await db
        .update(zuvySessions)
        .set(updateData)
  ...
  private async handleOldSessions(session: any) {
    if (
      ...
      const updateData: any = { s3link: 'not found' };
      await db
        .update(zuvySessions)
        .set(updateData)
        .where(eq(zuvySessions.id, session.id));
  ...
  private async getAttendanceByBatchId(batchId, creatorEmail: string) {
    try {
      ...
      const meetings = await db
        ...
        .where(
          and(
            eq(zuvySessions.batchId, Number(batchId)),
            eq(zuvySessions.status, 'completed'),
          ),
        );

      const [errorAttendance, attendance] = await this.calculateAttendance(
        client,
        meetings,
        students,
      );
  ...
  private async calculateAttendance(
    ...
    for (const meeting of meetings) {
      ...
      const organizerParam = items[0].events?.[0].parameters?.find(
        (p) => p.name === 'organizer_email',
      );
      ...
      const attendance: Record<
        string,
        { email: string; duration: number; attendance: string }
      > = {};
      ...
      response.data.items?.forEach((item: any) => {
        ...
        const email = e.parameters.find(
          (p: any) => p.name === 'identifier',
        )?.value;
        const secs =
          e.parameters.find((p: any) => p.name === 'duration_seconds')
            ?.intValue || 0;
